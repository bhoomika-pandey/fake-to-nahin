        ...
        child: StreamBuilder(
          ...
          builder: (context, snapshot) {
            ...
            return ListView.builder(
              ...
              itemBuilder: (context,index) => _buildPostViewScreen(context, snapshot.data.documents[index])
            );
          }
        )
      )
          ...
          padding: EdgeInsets.all(10),
            child: Column(children: [
              ...
              Container(
                ...
                child: Column(children:[
                  Row(children: [
                    Expanded(flex: 7,child: Text('Links to Resources',style: TextStyle(color:Colors.lightBlue[800],fontSize:22,fontWeight:FontWeight.bold),),),
                    Expanded(flex: 4,child: RaisedButton(onPressed: (){},child: Text('Post a Link',style: TextStyle(fontSize:20,color:Colors.white),),color: Colors.lightBlue[800],))
                  ],),
                  StreamBuilder(
                    stream: Firestore.instance.collection('post').document().collection('comments').snapshots(),
                    builder: (context,snapshot){
  ...
  Widget _buildCommentCards(BuildContext context,DocumentSnapshot document){
    return Container(
      ...
      child: Column(
        children:[
          Row(children: [
            (!document['imagePath']==null) ? (Image.asset('assets/img/as.png')):(Image.network(document['imagePath'])),
            Text(document['username'])
          ],),
          Row(children:[Text(document['dateCreated'])]),
          InkWell(onTap:()=>launch(document['link']),child: RichText(text: TextSpan(text: document['link'])),)
        ]
      ),
    );
  }
